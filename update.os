...
Процедура ОсновнаяОбработка()
	...
	Если Фильтр = "Top" Тогда
		Адрес = Адрес + "public/?sort=property_rating";
	ИначеЕсли Фильтр = "Download" Тогда
		Адрес = Адрес + "public/?sort=property_count_download";
	ИначеЕсли Фильтр = "Sale" Тогда
		Адрес = Адрес + "public/?sort=property_sale_sum";
	ИначеЕсли Фильтр = "Comment" Тогда
		Адрес = Адрес + "public/?sort=property_comments";
	ИначеЕсли Фильтр = "Show" Тогда
		Адрес = Адрес + "public/?sort=show_counter";
	ИначеЕсли ЗначениеЗаполнено(Фильтр) Тогда
		Сообщить("Фильтр не распознан. Будет показаны лучшие публикации. Возможные варианты фильтра: Top - Лучшие, Download - Топ-загрузок, Sale - Топ-продаж, Comment - Топ-комментариев, Show - Топ-просмотров, """" - Последние");
		Адрес = Адрес + "public/?sort=show_counter";
	КонецЕсли;
...
Процедура СформироватьИтоговыйТекст(Знач МассивДанных)
	...
	Для Н = 0 По Всего - 1 Цикл
		...
		Попытка
			...
			Для А = 0 По 7 Цикл
				ТекущийШаблон = СтрЗаменить(ТекущийШаблон, "%" + Строка(А + 1), МассивДанных[А + Н * 8])
			КонецЦикла;
			ВыходнойТекст = ВыходнойТекст + ТекущийШаблон;
				
		Исключение
			Сообщить("Публикация номер " + Строка(Н + 1) + " не сформировалась!");
			Продолжить;
		КонецПопытки;
